...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if let navVC = window?.rootViewController as? UINavigationController, let contactListVC = navVC.topViewController as? ContactsTableViewController {
            let networkTask = NetworkTask()
            let interactor = ContactsInteractor(networkTask: networkTask)
            let presenter = ContactsPresenter(interactor: interactor)
            presenter.delegate = contactListVC
